...
function ResumePDFPreviewImpl({ resume, slots, sections }: Props) {
  ...
  // PDFViewer regenerates the whole document whenever its `children` element
  // changes identity, so this element must stay referentially stable.
  const doc = useMemo(
    () => <ResumePDF resume={resume} slots={slots} sections={sections} />,
    [resume, slots, sections]
  );
...
export const ResumePDFPreview = memo(ResumePDFPreviewImpl);
